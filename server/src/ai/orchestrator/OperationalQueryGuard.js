...
const OPERATIONAL_PATTERNS = [
  ...
  /\b(what|which|who|list|show|find|get)\b.{0,60}\b(contract|document|request|task|matter|legal|status|signed|signing|signature)\b/i,
  ...
  /\b(internal|external)\b.{0,40}\b(sla|request|legal|turnaround|matter|compliance)\b/i,
  /\b(team workload|who is handling|whos handling|who are handling)\b/i,
  /\b(workflow history|status history|timeline|audit trail)\b.{0,40}\b(request|matter|lr-)\b/i,
  /\b(notify|notification|inbox|alert)\b.{0,40}\b(unread|new|how many|count)\b/i,
  /\b(contract|document|task|request|matter)\b.{0,60}\b(status|progress|update|current|latest|pending|signed|approved)\b/i,
  ...
  /\b(tasks?|work)\b.{0,50}\b(in|linked to|for)\b.{0,30}\blegal requests?\b/i,
  /\blegal requests?\b.{0,50}\b(tasks?|work items?)\b/i,
  ...
  /\b(which|show|list|find|how many)\b.{0,40}\b(loan|grant|nda|msa|sow|moa|mou|vendor|employment|lease)\b.{0,30}\b(contract|agreement|request)\b/i,
  // Submitted / newly filed legal requests
  /\b(submitted|submission|submissions|came in|newly submitted)\b.{0,50}\b(legal request|legal requests|legal)\b/i,
  /\b(legal request|legal requests)\b.{0,50}\b(submitted|submission|came in|recently|new|newly)\b/i,
  /\b(new legal requests?|recent legal submissions?)\b/i,
  /\b(submitted (to|in) legal|submit to legal|departments? submit)\b/i,
  /\b(requested|request|requests|legal intake|intake)\b.{0,60}\b(legal request|legal requests|from legal|to legal|legal intake|departments?|people)\b/i,
  /\b(legal request|legal requests|legal intake|intake)\b.{0,60}\b(requested|request|requests|recent|new|came in)\b/i,
  /\b(what was requested|what got requested|what did departments request|what did people request|show requested items|show legal intake)\b/i,
  /\blegal requests?\b/i,
];
